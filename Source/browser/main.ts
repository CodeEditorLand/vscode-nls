...
import { localize, LocalizeInfo, Options, setPseudo } from "../common/common";
...
export function loadMessageBundle(_file?: string) {
	return function (
		key: string | number | LocalizeInfo,
		message: string,
		...args: any[]
	): string {
		if (typeof key === "number") {
			throw new Error(
				`Browser implementation does currently not support externalized strings.`,
			);
		} else {
